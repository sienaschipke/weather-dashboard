...
// This function takes the city name as an input and converts the city to 
const getLatAndLong = city => {
    ...
    fetch(apiUrl)
        ...
        if (response.ok) {
            ...
            getForecastData(data[0].lat, data[0].lon);
            getTodaysWeather(data[0].lat, data[0].lon);
            });
        } else {
...
const getTodaysWeather = (lat, lon) => {
    const apiUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=imperial&appid=ee7c96e3014aea6e9033db179b95b0cc`;
